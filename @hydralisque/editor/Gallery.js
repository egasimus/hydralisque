...
class Gallery {
  constructor (callback) {
    ...
    //   console.debug('got sketches', res.text, err)
    //   if(err) {
    //     console.debug('err getting sketches', err)
    //   } else {
  ...
  //  if(!base64Code) base64Code = searchParams.get('id') // backwards compatibility with earlier form of naming. id is now called code
    ...
    let code = ''
    console.debug("id", sketch_id, "code", base64Code)
    ...
    //   //   this.foundSketch = true
    //   // } else {
    //   //   console.debug('id not found', sketch_id)
  ...
  setToURL(params){
    ...
    // }
  //  console.debug('url params', url_params)
  ...
  setRandomSketch() {
    ...
    console.debug("examples length", this.examples, this.exampleIndex)
    ...
    //  console.debug('example is', this.examples[rand])
  ...
  shareSketch(code, hydra, name) {
    this.saveSketch(code, () => {
      console.debug("URL is", this.url, 'sketch is', this.current)
      hydra.getScreenImage((img) => {
        ...
        //  .query(query)
          .end((err, res) => {
            if(err) {
              console.debug('error postingimage', err)
            } else {
              console.debug('image response', res.text)
        ...
        //   // Uploaded.
        //   console.debug("uploaded", oEvent)
        // };
        // oReq.send(img);
      //  console.debug('got image', img)
      })
    })
  }
  ...
  saveSketch(code, callback) {
    let self = this
    //console.debug('saving in gallery', code)
    let base64 = this.encodeBase64(code)
  //  console.debug('code is', base64)
    ...
    console.debug('saving in gallery', query)
    request
      .post('/sketch')
      // .send({
      //   code: base64
      ...
      .end((err, res) => {
        if(err) {
          console.debug('error posting sketch', err)
          if(callback) callback(err)
        } else {
          console.debug('response', res.text)
        //  self.setToURL([ { label: 'sketch_id', value: res.text}, {label: 'code', value: base64} ])
          self.setSketch({
            sketch_id: res.text,
            code: base64
  ...
  getExampleById(id) {
    console.debug('looking for', id, this.examples, this.sketches)
